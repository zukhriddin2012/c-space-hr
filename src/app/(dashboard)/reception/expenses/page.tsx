...
export default function ExpensesPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.expenseTypeId) {
      errors.expenseTypeId = 'Expense type is required';
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Record and manage business expenses</p>
        </div>
      ...
      <Card>
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by subject or expense number..."
              value={searchQuery}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => { setFilterExpenseType(e.target.value); setPage(1); }}
              ...
              <div>
                ...
                >
                  ...
                  {EXPENSE_PAYMENT_METHODS_LIST.map((pm) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                expenses.map((expense) => (
                  <tr key={expense.id} className={expense.isVoided ? 'bg-gray-50 opacity-60' : 'hover:bg-gray-50'}>
                    <td className="px-4 py-3">
                      <span className="font-mono text-sm text-gray-900">{expense.expenseNumber}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-medium text-gray-900">{expense.subject}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span>{expense.expenseType?.icon}</span>
                        <span className="text-gray-700">{expense.expenseType?.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span>{expense.paymentMethod === 'cash' ? '💵' : '🏦'}</span>
                        <span className="text-gray-700 capitalize">{expense.paymentMethod}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="font-semibold text-red-600">{formatCurrency(expense.amount)}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{expense.expenseDate}</td>
                    <td className="px-4 py-3">
                      {expense.isVoided ? (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {!expense.isVoided && (
                          <button
                            onClick={() => { setSelectedExpense(expense); setShowVoidModal(true); }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 ${
                  formErrors.expenseTypeId ? 'border-red-300' : 'border-gray-200'
                }`}
                required
              >
              ...
              {formErrors.expenseTypeId && (
                <p className="mt-1 text-sm text-red-600">{formErrors.expenseTypeId}</p>
              )}
            </div>
            ...
            <div>
              ...
              <select
                ...
                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 ${
                  formErrors.paymentMethod ? 'border-red-300' : 'border-gray-200'
                }`}
                required
              >
                {EXPENSE_PAYMENT_METHODS_LIST.map((pm) => (
      ...
      <Modal
        ...
        onClose={() => { setShowViewModal(false); setSelectedExpense(null); }}
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-lg font-semibold">{selectedExpense.expenseNumber}</span>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <p className="font-medium">{selectedExpense.subject}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedExpense.expenseType?.icon} {selectedExpense.expenseType?.name}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Payment Method</p>
                <p className="font-medium capitalize">
                  {selectedExpense.paymentMethod === 'cash' ? '💵' : '🏦'} {selectedExpense.paymentMethod}
                </p>
              </div>
            ...
            {selectedExpense.isVoided && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <p className="text-sm text-red-600">
                  Reason: {selectedExpense.voidReason || 'No reason provided'}
                </p>
                {selectedExpense.voidedAt && (
                  <p className="text-xs text-red-500 mt-1">
                    Voided on: {new Date(selectedExpense.voidedAt).toLocaleString()}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="flex justify-end pt-4 border-t">
              <Button variant="secondary" onClick={() => { setShowViewModal(false); setSelectedExpense(null); }}>
                Close
              </Button>
            </div>
          </div>
      ...
      <Modal
        ...
        onClose={() => { setShowVoidModal(false); setSelectedExpense(null); setVoidReason(''); }}
        title="Void Expense"
        size="md"
      >
        {selectedExpense && (
          <div className="space-y-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                You are about to void expense <strong>{selectedExpense.expenseNumber}</strong> for{' '}
                <strong>{formatCurrency(selectedExpense.amount)}</strong>.
              </p>
              <p className="text-sm text-yellow-700 mt-1">
                This action cannot be undone. The expense will be marked as voided but will remain in records for auditing.
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              <Button
                type="button"
                variant="secondary"
                onClick={() => { setShowVoidModal(false); setSelectedExpense(null); setVoidReason(''); }}
              >
                Cancel
              </Button>
              <Button
              ...
              >
                {isSubmitting ? 'Voiding...' : 'Void Expense'}
              </Button>
            </div>
          </div>
        )}
