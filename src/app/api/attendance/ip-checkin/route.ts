...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get employee by telegram ID (including preferred_language, remote_work_enabled, and default_shift)
    const { data: employee, error: empError } = await supabaseAdmin
      .from('employees')
      .select('id, full_name, position, branch_id, preferred_language, remote_work_enabled, default_shift')
