...
export default function CandidateDetailModal({
  ...
  // Sync AI analysis when candidate data changes
  useEffect(() => {
    if (candidate.ai_analysis) {
      setAiAnalysis(candidate.ai_analysis);
    }
  ...
  const handleAnalyzeResume = async () => {
    if (!candidate.resume_file_path) {
      setAiError('No resume uploaded for this candidate');
      return;
    }
    ...
    try {
      const res = await fetch(`/api/candidates/${candidate.id}/analyze`, {
        method: 'POST',
      });
      ...
      setAiError(error instanceof Error ? error.message : 'Failed to analyze resume');
    } finally {
                  ...
                  <div className="px-4 pb-4 space-y-3">
                    ...
                    <div className="bg-white rounded-lg p-3 border border-purple-100">
                      ...
                      {aiAnalysis.analyzed_at && (
                        <p className="text-xs text-gray-400 mt-2">
                          Analyzed {formatDate(aiAnalysis.analyzed_at)}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="bg-white rounded-lg p-3 border border-purple-100">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-sm">Role Fit</span>
                        </div>
                        <div className={`text-lg font-bold ${
                          aiAnalysis.role_fit.score >= 75 ? 'text-green-600' :
                          aiAnalysis.role_fit.score >= 50 ? 'text-yellow-600' : 'text-red-600'
                        }`}>
                          {aiAnalysis.role_fit.score}%
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                        <div
                          className={`h-2 rounded-full ${
                            aiAnalysis.role_fit.score >= 75 ? 'bg-green-500' :
                            aiAnalysis.role_fit.score >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          style={{ width: `${aiAnalysis.role_fit.score}%` }}
                        />
                      </div>
                    ...
                    <div className="bg-white rounded-lg border border-purple-100 overflow-hidden">
                      ...
                      {expandedSections.skills && (
                        <div className="px-3 pb-3 space-y-2">
                          {aiAnalysis.skills.technical.length > 0 && (
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Technical</p>
                              <div className="flex flex-wrap gap-1">
                                {aiAnalysis.skills.technical.map((skill, i) => (
                          ...
                          {aiAnalysis.skills.soft.length > 0 && (
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Soft Skills</p>
                              <div className="flex flex-wrap gap-1">
                                {aiAnalysis.skills.soft.map((skill, i) => (
                          ...
                          {aiAnalysis.skills.languages.length > 0 && (
                            <div>
                              <p className="text-xs text-gray-500 mb-1">Languages</p>
                              <div className="flex flex-wrap gap-1">
                                {aiAnalysis.skills.languages.map((lang, i) => (
                    ...
                    <div className="bg-white rounded-lg border border-purple-100 overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-sm">
                            Experience ({aiAnalysis.experience.total_years} years)
                          </span>
                        </div>
                      ...
                      {expandedSections.experience && (
                        <div className="px-3 pb-3 space-y-2">
                          {aiAnalysis.experience.companies.map((company, i) => (
                            <div key={i} className="border-l-2 border-purple-200 pl-3">
                              <p className="font-medium text-sm">{company.role}</p>
                              <p className="text-xs text-gray-500">{company.name} • {company.duration}</p>
                              {company.highlights.length > 0 && (
                                <ul className="mt-1 space-y-0.5">
                                  {company.highlights.slice(0, 2).map((h, j) => (
                    ...
                    <div className="bg-white rounded-lg border border-purple-100 overflow-hidden">
                      ...
                      {expandedSections.education && (
                        <div className="px-3 pb-3 space-y-2">
                          {aiAnalysis.education.map((edu, i) => (
                            <div key={i}>
                    ...
                    {aiAnalysis.red_flags.length > 0 && (
                      <div className="bg-white rounded-lg border border-red-100 overflow-hidden">
                        <button
                          onClick={() => toggleSection('red_flags')}
                          className="w-full flex items-center justify-between p-3 hover:bg-gray-50"
                        >
                          <div className="flex items-center gap-2">
                    ...
                    <div className="bg-white rounded-lg border border-purple-100 overflow-hidden">
                      <button
                        onClick={() => toggleSection('interview_questions')}
                        className="w-full flex items-center justify-between p-3 hover:bg-gray-50"
                      >
                        <div className="flex items-center gap-2">
                          <HelpCircle size={16} className="text-purple-600" />
                          <span className="font-medium text-sm">
                            Interview Questions ({aiAnalysis.interview_questions.length})
                          </span>
                        </div>
                        {expandedSections.interview_questions ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                      </button>
                      {expandedSections.interview_questions && (
                        <div className="px-3 pb-3 space-y-2">
                          {aiAnalysis.interview_questions.map((q, i) => (
                    ...
                    {/* Company Research */}
                    {aiAnalysis.company_research.length > 0 && (
                      <div className="bg-white rounded-lg border border-purple-100 overflow-hidden">
                        <button
                          onClick={() => toggleSection('company_research')}
                          className="w-full flex items-center justify-between p-3 hover:bg-gray-50"
                        >
                          <div className="flex items-center gap-2">
                          ...
                          {expandedSections.company_research ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                        </button>
                        {expandedSections.company_research && (
                          <div className="px-3 pb-3 space-y-2">
                            {aiAnalysis.company_research.map((company, i) => (
                              <div key={i} className="border-l-2 border-gray-200 pl-3">
                                <p className="font-medium text-sm">{company.company}</p>
                                <p className="text-xs text-gray-500">{company.industry}</p>
                                <p className="text-xs text-gray-600 mt-1">{company.insights}</p>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-lg p-3 border border-green-100">
                        <p className="text-xs text-green-600 font-medium mb-2">Strengths</p>
                        <ul className="space-y-1">
                          {aiAnalysis.role_fit.strengths.slice(0, 4).map((s, i) => (
                      ...
                      <div className="bg-white rounded-lg p-3 border border-orange-100">
                        <p className="text-xs text-orange-600 font-medium mb-2">Gaps</p>
                        <ul className="space-y-1">
                          {aiAnalysis.role_fit.gaps.slice(0, 4).map((g, i) => (
                            <li key={i} className="text-xs text-gray-600 flex items-start gap-1">
